...
def update_homework(request, pk=None):
    ...
    homework.is_finished = homework.is_finished != True
    homework.save()
    return redirect('homework')
...
def update_todo(request, pk=None):
    ...
    todo.is_finished = todo.is_finished != True
    todo.save()
    return redirect('todo')
...
def dictionary(request):
    if request.method=="POST":
        ...
        if 'phonetics' in answer[0] and 'text' in answer[0]['phonetics'][0]:
            phonetics = answer[0]['phonetics'][0]['text']
        else:
            phonetics = "N/A"  # or some default value if the key is not found
